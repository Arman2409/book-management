...
export class AuthService {
  ...
  async signUp({
    ...
    name,
    password,
  }: SignUpBody): Promise<{ access_token: string }> {
    try {
      if (!name) {
        throw new HttpException(
          'Field name is required for user creation',
      ...
      if (existingUser) {
        throw new HttpException(
          'User wtih the provided email already exists',
          HttpStatus.CONFLICT,
        );
      }
  ...
  async signIn({
    email,
    password,
  }: SignInBody): Promise<{ access_token: string } | undefined> {
    try {
      this.validator.validateEmailAndPassword(email, password);
